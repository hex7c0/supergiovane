...
function bootstrap(my) {
    ...
    my.mamma && require("mamma").createClient(my.mamma, process.pid).on("error", function(err) {
    ...
    if (app.get("/:pkg/:extra?", function(req, res, next) {
        ...
        if (my.cache !== !1 && void 0 !== STORY[hash]) return res.set("Content-Type", STORY[hash].content), 
        res.status(202).send(STORY[hash].body), void (Date.now() - STORY[hash].time > my.flush && delete STORY[hash]);
...
try {
    var cluster = require("cluster"), http = require("http"), resolve = require("path").resolve, status = http.STATUS_CODES, express = require("express"), logger = require("logger-request"), semver = require("semver");
} catch (MODULE_NOT_FOUND) {
    console.error(MODULE_NOT_FOUND), process.exit(1);
}

var VERSION = JSON.parse(require("fs").readFileSync(__dirname + "/package.json"));
...
module.exports = function(opt) {
    ...
    if (my.debug && (debug = logger({
        ...
        if (my.task && require("task-manager")(my.task, {
            output: Boolean(my.debug)
        }), "production" != my.env) return bootstrap(my);
