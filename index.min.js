...
function bootstrap(my) {
    ...
    if (app.get("/api/:pkg/:extra?", function(req, res, next) {
        var isBadge = !1, version = "/", pkg = req.params.pkg, extra = req.params.extra || "", style = req.query.style ? "?style=" + req.query.style : "", hash = pkg + extra + style;
        ...
        var val = validate(pkg);
        if (!val.validForNewPackages && !val.validForOldPackages) return next(new Error(status[404]));
        if ("" !== extra) if ("badge.svg" === extra) isBadge = !0; else {
            if (!semver(extra)) return next(new Error(status[404]));
            version += extra;
        }
        https.get({
            ...
            if (200 !== response.statusCode) return next(new Error(status[404]));
            var body = new Buffer(0);
            response.on("data", function(chunk) {
                body = Buffer.concat([ body, chunk ]);
            }).on("end", function() {
                try {
                    "gzip" === response.headers["content-encoding"] && (body = zlib(body)), body = JSON.parse(body);
                } catch (err) {
                    return next(new Error(status[502]));
                }
                if (body.readme = null, body.keywords = [], body.contributors = [], body.users = {}, 
                isBadge === !0) {
                    ...
                    return https.get({
                        ...
                        response.on("data", function(chunk) {
                            badge = Buffer.concat([ badge, chunk ]);
                        }).on("end", function() {
...
var cluster = require("cluster"), https = require("https"), http = require("http"), resolve = require("path").resolve, status = http.STATUS_CODES, express = require("express"), logger = require("logger-request"), semver = require("semver").valid, setHeader = require("setheaders").setWritableHeader, validate = require("validate-npm-package-name"), zlib = require("zlib").unzipSync, VERSION = JSON.parse(require("fs").readFileSync(__dirname + "/package.json"));
